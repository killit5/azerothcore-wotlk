...
#ifdef PRESETS
std::vector<Item*> Reforge::GetItemList(const Player* player)
{
...
void Reforge::RemoveReforge(Player* player, uint32 itemguid, bool update)
{
    if (!itemguid || reforgeMap.empty() ||
        reforgeMap.find(itemguid) == reforgeMap.end())
        return;
    ...
    reforgeMap.erase(itemguid);
    if (invItem && invItem->IsEquipped())
        player->_ApplyItemMods(invItem, invItem->GetSlot(), true);
    if (invItem)
        SendReforgePacket(player, invItem->GetEntry(),0,NULL);
}

void Reforge::SendReforgePackets(Player* player)
{
    ...
    for (std::vector<Item*>::const_iterator it = items.begin(); it != items.end(); ++it)
        SendReforgePacket(player, (*it)->GetEntry(), (*it)->GetGUID().GetCounter(),NULL);
}

// Supply lowguid or reforge! (or both)
// Warning, this function may modify reforgeMap when lowguid is supplied
void Reforge::SendReforgePacket(Player* player, uint32 entry, uint32 lowguid, const ReforgeData* reforge)
{
    ...
    if (lowguid)
    {
        if (!reforgeMap.empty() && reforgeMap.find(lowguid) != reforgeMap.end())
            reforge = &reforgeMap[lowguid];
        else
